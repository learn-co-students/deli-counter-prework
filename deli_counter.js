...
function nowServing(katzDeliLine){
  if (katzDeliLine.length>0){ //Makes sure that at least 1 person is waiting in line.
    ...
    var currentCustomer="Currently serving " + katzDeliLine[0] + "order number#"+ count + "."; //concats who is currently being served.
    katzDeliLine.shift(); //removes the person that was concatenated under the var=currentCustomer above.
    count ++;
    dailyServed=count;
    return currentCustomer, dailyServed;
  }
  else{
...
function customerTime(dailyServed){
  var date=Date.now();
  var timeTicket= customer +". "+ date + "total served: " + dailyServed;
